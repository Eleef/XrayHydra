...
*   左侧栏现在统一叫 `节点组`，同时展示两类来源：
    *   `订阅组`：来自 URL，可刷新、可删除。
    *   `自定义组`：手工创建的快照组，可重命名、删除、粘贴导入节点。
*   顶部 `新建节点组` 按钮会先弹出入口选择，再进入：
    *   `添加订阅组`
    *   `新建自定义组`
*   节点工具栏新增 `加入到分组`，可把当前勾选节点复制到已有自定义组，或在弹窗中直接输入新分组名称后创建并复制。
*   已经加入代理池的节点虽然会自动勾选并禁用，但节点行仍提供 `复制到分组`，方便把这些节点单独沉淀到自定义组。
*   自定义组节点是连接快照，不依赖原订阅继续存在；订阅刷新或删除不会影响已复制到自定义组的节点。
*   自定义组当前支持：
    *   粘贴多行 `vmess://` / `vless://` / `ss://` / `trojan://` 导入
    *   混合内容导入时返回 `ignored_unsupported_count`，明确提示被忽略的不支持协议节点数
    *   从当前节点列表批量复制
    *   组内语义去重
    *   组内单节点移除
...
* 租约页的活跃租约和冷却条目都会显示 `用 / 成 / 败` 三项小号统计，统计维度固定为 `workspace + proxy_port`，并持久化到 `data/lease_metrics.json`。
* 具体 workspace 视图下提供“复位 workspace”按钮；复位时可选择是否同时清空该 workspace 的租约统计，不勾选则保留历史计数。
